...
// Kaarten met cijfers
    ...
    const bar1 = createBar((totalCountries / maxValue) * 100, "bg-primary-subtle border border-primary mt-4");
    const bar2 = createBar((averagePopulation / maxValue) * 100, "bg-success-subtle border border-success mt-4");
    const bar3 = createBar((favoritesPopulation / maxValue) * 100, "bg-warning-subtle border border-warning mt-4");
...
function createBar(percentage, colorClass) {
    const barWrapper = createElement("div", "col bar-wrapper");
    const bar = createElement(
        "div",
        `rounded w-100 ${colorClass}`
    );
